...
internal func debugStr(
    ...
    var s: String
    if let debugPrintable = x as? CustomDebugStringConvertible { s = debugPrintable.debugDescription } else { s = "\(x)" }

    if consolidateWhitespace { s = s.replacing(regex: whitespacePat, with: " ") }

    if let truncate = truncate, s.count > truncate { s = s.prefix(truncate) + "…" }
...
extension Pipeline {
    internal func dump(_ indent: String = "") -> String {
        ...
        for stageKey in order {
            ...
            if stage.transformers.isEmpty { result += " (no transformers)" }
            for transformer in stage.transformers { result += "\n" + indent + "╟   " + debugStr(transformer) }
            if let cacheBox = stage.cacheBox { result += "\n" + indent + "╟─→ " + cacheBox.description }
            }
        return result
        }
    }
...
extension Entity {
    ...
    private var formattedContent: String {
        if JSONSerialization.isValidJSONObject(content),
           ...
           let json = String(data: jsonData, encoding: String.Encoding.utf8) { return json }
...
extension RequestError {
    internal func dump(_ indent: String = "") -> String {
        ...
        if httpStatusCode != nil { result += "\n" + indent + "httpStatusCode: \(debugStr(httpStatusCode))" }
        if cause != nil { result += "\n" + indent + "cause:          \(debugStr(cause, consolidateWhitespace: true))" }
        if let entity = entity { result += "\n" + indent + "entity:" + entity.dump(indent + "  ") }
        return result
        }

    internal func summary() -> String {
        var result = debugStr(userMessage, truncate: 24)
        if let httpStatusCode = httpStatusCode { result += " \(httpStatusCode)" }
        if let content = entity?.content { result += " content: \(type(of: content))" }
        if let cause = cause { result += " cause: " + debugStr(cause, truncate: 32)}
        return result
        }
    }
