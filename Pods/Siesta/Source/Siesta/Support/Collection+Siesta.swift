...
internal extension Collection {
    ...
    func any(match predicate: (Iterator.Element) -> Bool) -> Bool { return contains(where: predicate) }

    func all(match predicate: (Iterator.Element) -> Bool) -> Bool { return !contains(where: { !predicate($0) }) }
    }
...
internal extension Dictionary {
    static func fromArray<K, V>(_ arrayOfTuples: [(K, V)]) -> [K: V] {
        ...
        for (k, v) in arrayOfTuples { dict[k] = v }
        return dict
        }
    ...
    func flatMapDict<MappedKey, MappedValue>(transform: (Key, Value) -> (MappedKey?, MappedValue?))
        ...
        return Dictionary.fromArray(
            flatMap {
                ...
                if let k = k, let v = v { return (k, v) } else { return nil }
                }
            )
        }
