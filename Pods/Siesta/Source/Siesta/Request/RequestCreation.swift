    ...
    */
    public func request(
        ...
        func urlEscape(_ string: String) throws -> String {
            guard let escaped = string.addingPercentEncoding(withAllowedCharacters: Resource.allowedCharsInURLEncoding) else { throw RequestError.Cause.NotURLEncodable(offendingString: string) }
...
private final class FailedRequest: RequestWithDefaultCallbacks {
    ...
    init(error: RequestError) { self.error = error }
    ...
    func addResponseCallback(_ callback: @escaping ResponseCallback) -> Self {
        ...
        DispatchQueue.main.async { callback(ResponseInfo(response: .failure(self.error))) }
    ...
    func start() -> Self { return self }

    func cancel() { DispatchQueue.mainThreadPrecondition() }

    func repeated() -> Request { return self }
    }
